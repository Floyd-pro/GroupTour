...
//    private String endTime;
    ...
    private OnTimePickSubmitListener timePickSubmitListener;
    private OnTimePickCancelListener timePickCancelListener;
    private OnDatePickSubmitListener datePickSubmitListener;
    private OnDatePickCancelListener datePickCancelListener;
    private int confirmColor;
    private String cancel;

    @Override
    public int getLayoutId() {
        return R.layout.fragment_dialog_datetime;
    }
    ...
    public void initView(Bundle savedInstanceState) {
        ...
        pickerDate = findViewById(R.id.picker_date);
...
//        pickerEnd = findViewById(R.id.picker_end);
    ...
    public void initListener() {
        ...
        pickerDate.init(Integer.parseInt(preDate.substring(0, preDate.indexOf('-'))),
                Integer.parseInt(preDate.substring(preDate.indexOf('-')+1, preDate.lastIndexOf('-'))) - 1,
                Integer.parseInt(preDate.substring(preDate.lastIndexOf('-')+1, preDate.length())), new DatePicker.OnDateChangedListener() {
            @Override
            public void onDateChanged(DatePicker datePicker, int i, int i1, int i2) {
                date = getDate(i, i1 + 1, i2);
            }
        });
...
//        });
        ...
        btnCancel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(datePickCancelListener != null && timePickCancelListener != null) {
                    datePickCancelListener.onClickCancel(v);
                    timePickCancelListener.onClickCancel(v);
                }
                dismiss();
            }
        });
        ...
        btnSubmit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(datePickSubmitListener != null && timePickSubmitListener != null) {
                    datePickSubmitListener.onClickSubmit(v, date);
                    timePickSubmitListener.onClickSubmit(v, time);
                }
                dismiss();
            }
        });
    ...
    private String getTime(int hour, int minute) {
        ...
        return showMinute ?  h + ":" + m + ":00" : h+":00:00";
    }

    private String getDate(int year, int month, int day) {
        String y = String.valueOf(year);
        String m = getDateDoubleDigit(month);
        String d = getDateDoubleDigit(day);
        return y + "-" + m + "-" + d + " ";
    }
    ...
    private String getDateDoubleDigit(int date) {
...
//        pickerStart.setDescendantFocusability(TimePicker.FOCUS_BLOCK_DESCENDANTS);
        pickerTime.setDescendantFocusability(TimePicker.FOCUS_BLOCK_DESCENDANTS);

//        setTimePickerDividerColor(pickerStart);
        setTimePickerDividerColor(pickerTime);
//        hideMinute(pickerStart);
        hideMinute(pickerTime);

        Calendar calendar = Calendar.getInstance();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
//            pickerStart.setHour(calendar.get(Calendar.HOUR_OF_DAY));
//            pickerStart.setMinute(calendar.get(Calendar.MINUTE));
            pickerTime.setHour(calendar.get(Calendar.HOUR_OF_DAY));
            pickerTime.setMinute(calendar.get(Calendar.MINUTE));
...
//            pickerStart.setCurrentMinute(calendar.get(Calendar.MINUTE));
        ...
        if(!TextUtils.isEmpty(preDate) && !TextUtils.isEmpty(preTime)) {
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
...
//                pickerStart.setMinute(Integer.parseInt(preStartTime.substring(preStartTime.indexOf(":") + 1)));
                ...
                pickerTime.setHour(Integer.parseInt(preTime.substring(0, preTime.indexOf(":"))));
                pickerTime.setMinute(Integer.parseInt(preTime.substring(preTime.indexOf(":") + 1)));
            } else {
...
//                pickerStart.setCurrentMinute(Integer.parseInt(preStartTime.substring(preStartTime.indexOf(":") + 1)));
                ...
                pickerTime.setCurrentHour(Integer.parseInt(preTime.substring(0, preTime.indexOf(":"))));
                pickerTime.setCurrentMinute(Integer.parseInt(preTime.substring(preTime.indexOf(":") + 1)));
            }
        }

        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
//            startTime = getTime(pickerStart.getHour(), pickerStart.getMinute());
            time = getTime(pickerTime.getHour(), pickerTime.getMinute());
        }else {
//            startTime = getTime(pickerStart.getCurrentHour(), pickerStart.getCurrentMinute());
            time = getTime(pickerTime.getCurrentHour(), pickerTime.getCurrentMinute());
        }
    }
    ...
    public interface OnTimePickSubmitListener {
        void onClickSubmit(View view, String time);
    }
    
    public interface OnDatePickCancelListener {
        void onClickCancel(View view);
    }
    
    public interface OnDatePickSubmitListener {
        void onClickSubmit(View view,String date);
    }
    ...
    public static class Builder {
        ...
        public Builder setOnTimePickSubmitListener(@StringRes int confirm, OnTimePickSubmitListener listener) {
            ...
            this.timePickSubmitListener = listener;
            return this;
        }
        ...
        public Builder setOnDatePickSubmitListener(@StringRes int confirm, OnDatePickSubmitListener listener) {
            this.confirmText = context.getString(confirm);
            this.datePickSubmitListener = listener;
            return this;
        }

        public Builder setOnDatePickSubmitListener(String confirm, OnDatePickSubmitListener listener) {
            this.confirmText = confirm;
            this.datePickSubmitListener = listener;
            return this;
        }

        public Builder setOnDatePickSubmitListener(OnDatePickSubmitListener listener) {
            this.datePickSubmitListener = listener;
            return this;
        }
        ...
        public Builder setOnTimePickCancelListener(@StringRes int cancel, OnTimePickCancelListener listener) {
            ...
            this.timePickCancelListener = listener;
            return this;
        }
        ...
        public Builder setOnDatePickCancelListener(@StringRes int cancel, OnDatePickCancelListener listener) {
            this.cancel = context.getString(cancel);
            this.datePickCancelListener = listener;
            return this;
        }

        public Builder setOnDatePickCancelListener(String cancel, OnDatePickCancelListener listener) {
            this.cancel = cancel;
            this.datePickCancelListener = listener;
            return this;
        }

        public Builder setOnDatePickCancelListener(OnDatePickCancelListener listener) {
            this.datePickCancelListener = listener;
            return this;
        }
...
//        }
        ...
        public DatetimeDialogFragment build() {
            DatetimeDialogFragment fragment = getFragment();
            ...
            fragment.setOnConfirmClickListener(this.timePickSubmitListener, this.datePickSubmitListener);
            fragment.setConfirmColor(confirmColor);
            fragment.setCancelText(cancel);
            fragment.setOnCancelClickListener(timePickCancelListener, datePickCancelListener);
...
//            fragment.setEndTime(endTime);
        ...
        protected DatetimeDialogFragment getFragment() {
            return new DatetimeDialogFragment();
        }

        public DatetimeDialogFragment show() { DatetimeDialogFragment fragment = build();
...
//    }
    ...
    private void setOnConfirmClickListener(OnTimePickSubmitListener timeListener, OnDatePickSubmitListener dateListener) {
        this.timePickSubmitListener = timeListener;
        this.datePickSubmitListener = dateListener;
    }
    ...
    private void setOnCancelClickListener(OnTimePickCancelListener timeListener, OnDatePickCancelListener dateListener) {
        this.timePickCancelListener = timeListener;
        this.datePickCancelListener = dateListener;
    }
}