...
public class RegisterFragment extends BaseInitFragment implements TextWatcher, View.OnClickListener, EaseTitleBar.OnBackPressListener, CompoundButton.OnCheckedChangeListener {
    ...
    ExecutorService executorService = Executors.newSingleThreadExecutor(); // 用于新用户注册信息更新到数据库
        ...
        mViewModel.getRegisterObservable().observe(this, response -> {
            parseResource(response, new OnResourceParseCallback<String>(true) {
                ...
                public void onSuccess(String data) {
                    ...
                    // TODO: 2021/4/17 在服务器上插入新用户数据
                    Person p = new Person(mEtLoginName.getText().toString(), mEtLoginPwd.getText().toString());
                    Future future = executorService.submit(new Callable() {
                        public Object call() throws Exception {
                            if(DBUtil.insertPerson(p)) {
                                return "插入用户数据成功！\n" + p.toString();
                            }
                            return "插入用户数据失败！";
                        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_login :
                if(mEtLoginName.getText().toString().trim().contains("admin") || mEtLoginName.getText().toString().trim().contains("admin")) {
                    mEtLoginName.setText("");
                    ToastUtils.showToast("包含违禁字符，请重新输入新的用户名！");
                    break;
                }
                registerToHx();
                break;
