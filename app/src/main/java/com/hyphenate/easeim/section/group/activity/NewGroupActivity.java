...
public class NewGroupActivity extends BaseInitActivity implements EaseTitleBar.OnBackPressListener, EaseTitleBar.OnRightClickListener, View.OnClickListener, SwitchItemView.OnCheckedChangeListener {
    ...
    ExecutorService executorService; // 用于新队伍注册信息更新到数据库
    ...
    protected void initIntent(Intent intent) {
        ...
        StatusBarUtils.setTranslucentStatusTextMode(NewGroupActivity.this, true);
        ...
        viewModel.groupObservable().observe(this, response -> {
            parseResource(response, new OnResourceParseCallback<EMGroup>() {
                ...
                public void onSuccess(EMGroup data) {
                    ...
                    // TODO: 2021/4/21 在数据库插入新队伍数据
                    Team t = new Team(itemGroupName.getTvContent().getText().toString(),
                            itemGroupProfile.getTvContent().getText().toString(),
                            DemoHelper.getInstance().getCurrentUser());

                    executorService = Executors.newSingleThreadExecutor();
                    Future future = executorService.submit(new Callable() {
                        public Object call() throws Exception {
                            if(DBUtil.insertTeam(t)) {
                                return "插入新队伍数据成功！\n" + t.toString();
                            }
                            return "插入新队伍数据失败！";
                        }
